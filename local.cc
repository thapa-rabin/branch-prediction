...
int main() {
    ...
    const int nBHRbits = 4; // number of bits in each BHR
    const int nBHR = 4;// number of BHR counters
    const int num_of_bits_needed_for_each_BHR = 2;
    
    SmithCounter c[nCounters]; //declare an array of SmithCounters
    
	int BHT[nBHR]; // Branch History Table
	//initially all BHRs set to 0
    ...
    if (myfile.is_open()) {	
        ...
        while (!myfile.eof()) { 	  //read address and outcome (one at a time) till the end of file
			...
			int index, indexPC, indexBHR;
			//index taken from Program Counter (1-bit)
			indexPC = maskAddress(address,num_of_bits_needed-nBHRbits+1,2); // return bits indexed 2 to num_of_bits_needed+1
			indexBHR = maskAddress(address,num_of_bits_needed-nBHRbits+num_of_bits_needed_for_each_BHR+1,2); //2-bit
			
			//index of Smith Counter
			index = (indexPC << nBHRbits) | BHT[indexBHR]; //bits from BHT appended to right of indexPC to form 5-bit index
            ...
            if(outcome == 'T') {
            	...
            	//update the current counter in BHT
            	//remove the leftmost bit and append 1 on the right 
            	BHT[indexBHR] = ((BHT[indexBHR] & ((1 << (nBHRbits-1))-1)) << 1) | 1;
            }
            else if(outcome == 'N') {
            	//compare prediction with outcome
            	if(c[index].getPrediction() == outcome)
            	...
            	//update the current counter in BHT
            	//remove the leftmost bit and append 1 on the right 
            	BHT[indexBHR] = (BHT[indexBHR] & ((1 << (nBHRbits-1))-1) << 1) | 0;
            }
