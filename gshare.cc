...
int main() {
    ...
    const int nBHRbits = 5; // number of bits in BHR
    ...
    if (myfile.is_open()) {	
        ...
        while (!myfile.eof()) { 	  //read address and outcome (one at a time) till the end of file
			...
			int index, indexPC;
			//index taken from Program Counter (5 bits)
			indexPC = maskAddress(address,num_of_bits_needed+1,2); // return bits indexed 2 to num_of_bits_needed+1
			
			//index of Smith Counter
			index = indexPC ^ BHR; // indexPC XORed with BHR
            ...
            if(outcome == 'T') {
            	...
            	//update BHR
            	BHR = ((BHR & ((1 << (nBHRbits-1))-1)) << 1) | 1;; // BHR left shifted one bit and appended 1 to the right
            }
            else if(outcome == 'N') {
            	//compare prediction with outcome
            	if(c[index].getPrediction() == outcome)
            	...
            	BHR = ((BHR & ((1 << (nBHRbits-1))-1)) << 1) | 0; // BHR left shifted one bit and appended 0 to the right
            }
